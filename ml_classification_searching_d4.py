...
# No read data with results below 98
wanted_pass_rate = 99.5
df['Target'] = (df['γ-index'] < wanted_pass_rate).astype(int)


df = df.replace({" nan": np.nan}).dropna()

# Define features and target
features = ['Avg_MCS', 'Avg_EM', 'Avg_LeafTravel',	'Avg_ArcLength']
...
best_feature = 'Avg_MCS'  # Replace with your actual best feature
...
print(f"When {best_feature} >= {optimal_cutoff:.4f}, predict Pass Rate < {wanted_pass_rate}")
logging.info(f"When {best_feature} >= {optimal_cutoff:.4f}, predict Pass Rate < {wanted_pass_rate}")